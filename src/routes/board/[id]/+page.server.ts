...
export const load: PageServerLoad = async ({ params, locals, platform, setHeaders }) => {
    ...
    const rankings = await getCachedData(platform?.caches, `rankings-${board.id}`,
        async () => await locals.DB
            .select()
            .from(table.rankings)
            .where(eq(table.rankings.boardId, boardId))
    );

    const resolvedOwner = await getCachedData(platform?.caches, `owner-${board.owner}`,
        async () => (await locals.DB
